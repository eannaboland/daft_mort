...
class Listing:
    ...
    def floor_area(self):
        """
        This method returns the properties floor area.
        :return:
        """
        try:
            if self.data_from_search:
                floor_area = self._ad_page_content.find("div",{"class":"PropertyOverview__propertyOverviewDetails"}).text
                fa = floor_area.splitlines()
                
                floor_area = [s for s in fa if "m2" in str(s)]              
                floor_area = str(floor_area).strip()
                floor_area = floor_area.strip("['m2']")

                return floor_area

        except Exception as e:
            logging.error("Error getting floor area. Error message: " + e.args[0])
            return
    ...
    def dwelling_type(self):
        ...
        try:
            if self.data_from_search:
                info = self.data_from_search.find("div", {"class": "QuickPropertyDetails__propertyType"}).text
                #s = info.split("|")
                #return s[0].strip()
                return info
            else:
    ...
    def ber_code(self):
        ...
        try:
            ...
            alt_text = self._ad_page_content.find("div", {"class": "PropertyImage__propertyInfoContainer"}).find(
                "img"
            ).attrs["src"]
            ...
            if "exempt" in alt_text:
                ...
                alt_arr = alt_text.split("/")[-2:]
                if "ber" in alt_arr[0].lower():
                    ber = re.search('ber_(.*).svg', alt_arr[1])
                    ber = (ber.group(1))
                    return ber.upper()
                else:
                    return None
