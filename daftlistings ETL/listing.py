...
class Listing(object):
    def __init__(self, data, verbose=False):
        ...
        self._is_new_themed = self._data.find('div', {'class': "PropertyImage__propertyInfoContainer"}) != None
    @property
    def id(self):
    ...
    def price(self):
        ...
        """
        try:
            if "PropertyCardContainer__container" in str(self._data):
                return self._data.find('strong', {'class': 'PropertyInformationCommonStyles__costAmountCopy'}).text
            else:
    ...
    def formalised_address(self):
        ...
        """
        try:
            if self._is_new_themed:
                t = self._data.find('a', "PropertyInformationCommonStyles__addressCopy--link").contents[0]
            else:
    ...
    def dwelling_type(self):
        ...
        try:
            if self._is_new_themed:
                return self._data.find('div', {'class': "QuickPropertyDetails__propertyType"}).text
    ...
    def bedrooms(self):
        ...
        try:
            if self._is_new_themed:
                return self._data.find('div', {'class': "QuickPropertyDetails__iconCopy"}).text
            info = self._data.find('ul', {"class": "info"}).text
            s = info.split('|')
            nb = s[1].strip()
    ...
    def bathrooms(self):
        ...
        try:
            if self._is_new_themed:
                return self._data.find('div', {'class': "QuickPropertyDetails__iconCopy--WithBorder"}).text
            info = self._datafind('ul', {"class": "info"}).text
            s = info.split('|')
            nb = s[2].strip()
            return int(nb.split()[0])
        except Exception as e:
